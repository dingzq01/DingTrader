#!/usr/bin/env python
"""MACD Bottom Divergence V1 组合回测 CLI (docs/plan12)。

用法:
    python scripts/run_macd_bottom_divergence_backtest.py --from=2024-01-01 --to=2024-12-31
    python scripts/run_macd_bottom_divergence_backtest.py --max-positions=5 --position-size=100000

流程:
    1) data_adapter 从 TimescaleDB 加载沪深主板股票池宽表数据
    2) MACDBottomDivergenceV1 规则判断 (macd_bottom_divergence 事件 / ma5<ma10)
       → engine 组合回测 (Backtrader, T+1 开盘执行, 禁止重复买入, 卖出全平)
...
from src.strategies.macd_bottom_divergence import MACDBottomDivergenceV1
...
def main():
    setup_logging()
    parser = argparse.ArgumentParser(description="MACD Bottom Divergence V1 组合回测")
    ...
    strategy = MACDBottomDivergenceV1()
